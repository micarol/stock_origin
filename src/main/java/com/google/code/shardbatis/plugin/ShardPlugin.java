...
public class ShardPlugin implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (isShouldParse(mapperId)) {
			...
			// change table name modify logic by frankie 2013-02-19
			sql = sql.trim();
			sql = sql.toLowerCase();
			sql = sql.replaceAll("\n", " ");
			sql = sql.replaceAll("\t", " ");
	...
	private String convert(String sql, Object params, String mapperId) {
		return sql;
	}
	
	public static void main(String[] arg) {
		String sql = "insert into xxx(xx,yy,zz) values(xxx, now(), now());";
		System.out.println(sql.replaceFirst("\\(", " ("));
